...
const db = require('quick.db')
    ...
    exports.run = (client, message, args) => {
        ...
        if(user && isim && yas){
            ...
            let kadın = db.get(`kadınpuan_${message.author.id}_${message.guild.id}`) 
            db.add(`kadınpuan_${message.author.id}_${message.guild.id}`, +1)

            const kayit = new Discord.MessageEmbed()
                .setColor('BLACK')
                .setDescription(` ${user} **Kişisi,** <@&Kadın Rol İD> **Olarak Kayıt Edildi\n Toplam Kayıt Sayın: ${kadın || 0}**`)
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['Bayan','B','bayan','BAYAN','girl','Girl','GİRL'],
    permLevel: 0
}
